...
func NewPostgreSQLRepository(host, uname, pass, dbname string, port int, config *gorm.Config) (*PostgreSQLClientRepository, error) {
	...
	maxIdleConn, errMaxIdleConn := strconv.Atoi(os.Getenv(util.CONFIG_DB_MERCHANT_MAX_IDLE_CONNECTION))
	util.IsErrorDoPanic(errMaxIdleConn)
	maxOpenConn, errMaxOpenConn := strconv.Atoi(os.Getenv(util.CONFIG_DB_MERCHANT_MAX_OPEN_CONNECTION))
	util.IsErrorDoPanic(errMaxOpenConn)
	lifetimeConn, errMaxLifetimeConn := time.ParseDuration(os.Getenv(util.CONFIG_DB_MERCHANT_MAX_LIFETIME_CONNECTION))
